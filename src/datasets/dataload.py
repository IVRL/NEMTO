...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, instance_dir):
        ...
        envmap_path = os.path.join(self.instance_dir, 'envmap.exr')
        print('Found # images, # masks, # cameras: ', len(image_paths), len(mask_paths), len(cam_dict))
        
        self.n_cameras = len(mask_paths)
        ...
        img = imageio.imread(envmap_path)[:, :, :3]
        img = np.float32(img)
        gamma = 2.2
        tonemap_img = lambda x: np.power(x, 1./gamma)
        self.envmap = torch.from_numpy(tonemap_img(img)).float()


    def __len__(self):
        return self.n_cameras

    def return_single_mask(self, img_name):
        self.single_imgname = img_name
        for idx in range(len(self.mask_paths)):
            if os.path.basename(self.mask_paths[idx]) == self.single_imgname:
